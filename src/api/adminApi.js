...
/**
 * 📞 PUT /api/admin/customers/{id}/toggle-lock
 * 🔒 Khóa/Mở khóa tài khoản khách hàng
 * 📥 Tham số: id - ID của khách hàng
 * 🔙 Trả về: Customer object với trạng thái mới
 * ⚠️ Lưu ý: Toggle isActive trong Account (true <-> false)
 */
export const toggleCustomerLock = async (id) => {
	try {
		return await axiosClient.put(`/admin/customers/${id}/toggle-lock`);
	} catch (err) {
		console.error('toggleCustomerLock error', err);
		throw err;
	}
};
